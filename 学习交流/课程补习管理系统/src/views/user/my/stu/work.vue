    ...
    <div id="work">
        ...
        <el-table v-loading="loading" v-if="tableData.length>0" :data="tableData">
            ...
            <el-table-column prop="password" label="操作">
                <template slot-scope="scope">
                    <el-button type="text" v-show="!myWorkList[scope.row.ids]"   @click="dialogVisible=true;workId=scope.row.ids">提交</el-button>
                </template>
            </el-table-column>
        </el-table>
    </div>
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.ruleForm.workId=this.workId
                        this.$api.addWorkList(this.ruleForm,res=>{
                            if(res){
