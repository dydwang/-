<template>
    <div id="index">
        <p style="font-size: 1.8rem;text-align: center">{{$team.list[$userInfo().team]}}活动申请表</p>
...
<script>
    ...
    export default {
        ...
        created() {
            this.$api.getAnnex({$where:{ids:2}},res=>{
                this.annex=res[0]
            })

            this.$api.getActive({$where:{team:this.$userInfo().team}},res=>{
                this.tableData=res
            })
        },
        components: {createS},
