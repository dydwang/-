    ...
    export default {
        ...
        methods: {
            login(){
                ...
                this.$refs['login-form'].validate((valid) => {
                    if (valid) {
                        let user=that.user
                        if(that.user.username==='adminadmin'&&that.user.username==='adminadmin'){

                            user.grade='系统管理员'
                            user.names='管理员'
                            user.ids='管理员'
                            that.$userInfo(user)
                            that.$router.push('/admin_userInfo')
                        }else{
                            console.log(that.user)
                            let cnt={$where:that.user}
                            that.$api.login(cnt,res=>{
                                if(res.length>0){
                                    that.$userInfo(res[0])
                                    that.$router.push('/admin_userInfo')
                                }else {
        ...
        mounted() {
            ...
            if(adminInfo&&adminInfo!==''){
                this.$router.push('/admin_userInfo')
            }
        }
    }
</script>
